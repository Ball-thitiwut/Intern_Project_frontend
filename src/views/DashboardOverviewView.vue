<template>
  <div
    class="space-y-12 md:space-y-20 font-sans text-[#051960] animate-fade-in pb-20"
  >
    <div
    ...
    <div
      ...
      class="flex flex-col justify-center items-center h-40 text-red-500"
    >
      <p>
        {{
          $t("dashboard_view.error_message") || "เกิดข้อผิดพลาดในการดึงข้อมูล"
        }}
      </p>
      <button @click="fetchData" class="mt-2 text-sm underline text-blue-600">
        ลองใหม่อีกครั้ง
      </button>
    </div>

    <div v-else class="space-y-12 md:space-y-20">
      <section class="space-y-4 md:space-y-6">
        <div class="flex items-center gap-3 px-2 border-l-4 border-blue-600">
          <h2 class="text-xl md:text-2xl font-bold uppercase tracking-wide">
            {{ $t("filter_bar.views.sales") }}
          </h2>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-10 gap-4 md:gap-6">
          ...
          >
            ...
            <div class="grid grid-cols-1 gap-2 w-full mt-4">
              <div
                class="flex justify-between items-center bg-gray-50 p-2 px-4 rounded-xl border border-gray-100 text-sm"
              >
                      ...
                      >
                        ...
                        <div class="space-y-1">
                          <p class="text-gray-500 font-bold text-base">
                            {{
                              $t(
                                "dashboard_view.top_menu_table.no_data_title",
                              ) || "ไม่พบข้อมูลการขาย"
                            }}
                          </p>
      ...
      <section class="space-y-4 md:space-y-6">
        <div class="flex items-center gap-3 px-2 border-l-4 border-green-600">
          <h2 class="text-xl md:text-2xl font-bold uppercase tracking-wide">
            {{ $t("filter_bar.views.bill") }}
          </h2>
        </div>
        <div
          class="bg-white rounded-2xl md:rounded-3xl p-4 md:p-6 md:pb-10 shadow-sm w-full border border-gray-100"
        >
          <div
            class="text-base md:text-xl font-bold text-[#051960] mb-4 md:mb-6"
          >
      ...
      <section class="space-y-4 md:space-y-6 pb-20">
        <div class="flex items-center gap-3 px-2 border-l-4 border-purple-600">
          <h2 class="text-xl md:text-2xl font-bold uppercase tracking-wide">
            {{ $t("filter_bar.views.behavior") }}
          </h2>
        </div>
...
const groupSizeTableData = computed(() => {
  ...
  return masterGroups.map((groupLabel) => {
    const found = raw.find(
      (item) =>
        item.label.replace(/People|Person|persons|people/gi, "").trim() ===
        groupLabel,
    );
    return {
      size: groupLabel,
...
const calcYTicks = (data) => {
  ...
  const ticks = [];
  for (let i = 0; i <= 5; i++) ticks.push((step * i).toString());
...
const isHourlyView = computed(() => {
  ...
  if (props.dateRange?.[0] && props.dateRange?.[1]) {
    return (
      new Date(props.dateRange[0]).setHours(0, 0, 0, 0) ===
      new Date(props.dateRange[1]).setHours(0, 0, 0, 0)
    );
  }
  return false;
});
...
onMounted(() => fetchData());
watch([() => props.dateRange, () => props.period], () => fetchData());
</script>
