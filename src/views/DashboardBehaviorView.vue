<template>
  <div class="space-y-6 font-sans text-white">
    <FilterBar
      :initialPeriod="'1m'"
      :initialView="'behavior'"
    ...
    <div class="bg-white rounded-2xl p-6 shadow-sm">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-5 gap-8">
        <div
          class="lg:col-span-3 h-64 flex flex-col justify-end relative pl-8 pr-4"
        >
          <div
            class="absolute left-0 top-0 bottom-8 flex flex-col justify-between text-xs text-gray-400 h-full pb-2"
          >
            <span>70</span>
            <span>60</span>
            <span>50</span>
          ...
          <div
            class="absolute -left-8 top-1/2 -translate-y-1/2 -rotate-90 text-xs text-gray-500"
          >
            จำนวนบิล
          </div>

          <div
            class="flex items-end justify-between gap-4 h-full border-b border-gray-100 pb-2 relative z-10"
          >
            <div
              class="absolute inset-0 flex flex-col justify-between pointer-events-none z-0 pb-2"
            >
              <div
                v-for="i in 8"
                :key="i"
                class="border-t border-gray-100 border-dashed w-full h-0"
              ></div>
            </div>

            <div
              v-for="(item, index) in groupSizeChart"
              :key="index"
              class="flex flex-col items-center w-full group relative z-10"
            >
              <div
                v-if="item.highlight"
                class="mb-2 bg-[#051960] text-white text-[10px] font-bold px-2 py-1 rounded-md shadow-lg relative -top-1"
              >
                {{ item.value }}
                <div
              ...
              <span class="mt-3 text-xs text-gray-500 font-medium">{{
                item.label
              }}</span>
            </div>
          </div>
        ...
        <div
          class="lg:col-span-2 bg-white rounded-xl px-5 py-3 shadow-sm flex flex-col h-full border border-gray-100"
        >
          <table class="w-full text-sm">
            ...
            <tbody class="text-sm text-gray-600">
              <tr
                v-for="(row, idx) in groupSizeTable"
                :key="idx"
              ...
              >
                ...
                <td class="py-2.5 text-right font-bold text-[#051960]">
                  {{ row.count }} บิล
                </td>
          ...
          <div
            class="mt-auto pt-4 text-right text-base font-bold text-[#051960]"
          >
            รวมทั้งหมด: 176 บิล
          </div>
        </div>
    ...
    <div class="bg-white rounded-2xl p-6 shadow-sm">
      ...
      <div
        class="flex items-center justify-center mb-8 pb-6 border-b border-gray-200"
      >
      ...
      <div class="h-56 flex flex-col justify-end relative pl-8 pr-4">
        ...
        <div
          class="flex items-end justify-around h-full relative z-10 border-b border-gray-100 pb-2"
        >
          <div
            class="absolute inset-0 flex flex-col justify-between pointer-events-none z-0 pb-2"
          >
          ...
          <div
            v-for="(item, index) in spendPerPersonChart"
            :key="index"
          ...
          >
            ...
            ></div>
            <span
              class="mt-3 text-[10px] md:text-xs text-gray-500 font-medium"
              >{{ item.label }}</span
            >
          </div>
        </div>
    ...
    <div class="bg-white rounded-2xl p-6 shadow-sm">
      ...
      <div class="h-56 flex flex-col justify-end relative pl-8 pr-4">
        ...
        <div
          class="absolute left-0 top-0 bottom-8 flex flex-col justify-between text-xs text-gray-400 h-full pb-2"
        >
          <span>30</span><span>25</span><span>20</span><span>15</span
        ...
        >
          ...
          <div
            v-for="(item, index) in salesPerBillChart"
            :key="index"
            class="flex flex-col items-center w-full relative group"
          >
            <div
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        >
          <div class="flex items-center justify-between gap-2 mb-4">
            <div
              class="w-full aspect-square bg-[#002683]/5 rounded-xl shadow-sm border border-gray-100"
            ></div>
          ...
          <div class="flex justify-between items-end">
            <div class="space-y-1">
              <div class="text-sm font-semibold text-[#051960] leading-tight">
                {{ pair.name1 }} {{ pair.name2 }}
              </div>
              <div class="text-[12px] text-gray-500">จำนวนการสั่งร่วมกัน</div>
            </div>
            <div class="text-right">
              <div class="text-sm font-bold text-[#051960]">
                ฿{{ pair.price }}
              </div>
...
const handlePeriodChange = (p) => console.log("Period:", p);
const handleViewChange = (v) => console.log("View:", v);
</script>