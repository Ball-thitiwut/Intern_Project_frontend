<template>
  <div class="bg-white rounded-3xl p-6 shadow-sm border border-gray-100">
    <h3 class="text-xl font-bold text-[#051960] mb-6">จำนวนบิลตามขนาดกลุ่มลูกค้า</h3>

    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
      
      <div class="lg:col-span-7 h-72 relative mb-10">
        <div class="absolute -left-2 top-1/2 -translate-y-1/2 -rotate-90 text-xs text-gray-500 font-medium whitespace-nowrap z-20">
          จำนวนบิล
        </div>
        ...
        <div class="ml-24 h-full border-b border-gray-300 relative">
           <div class="absolute inset-0 flex flex-col justify-between pointer-events-none z-0 pb-[1px]">
            <div v-for="i in 8" :key="i" class="w-full h-0 border-t border-gray-100 border-dashed" :class="{ 'border-transparent': i === 8 }"></div>
          </div>
          <div class="flex items-end justify-around h-full relative z-10 px-2">
            <div v-for="(item, index) in chartData" :key="index" class="flex flex-col items-center w-full relative group h-full justify-end">
              ...
              <div class="w-full max-w-[50px] rounded-t-lg transition-all duration-300 relative" :class="item.highlight ? 'bg-[#F47122] shadow-md z-10' : 'bg-gray-100 hover:bg-gray-200'" :style="{ height: item.percent + '%' }"></div>
              <span class="absolute top-full mt-3 text-xs text-gray-500 font-medium text-center w-full">{{ item.label }}</span>
            </div>
          </div>
        </div>
        <div class="absolute -bottom-12 left-0 right-0 text-center text-xs text-gray-500 font-medium ml-24">ขนาดกลุ่มลูกค้า(คน)</div>
      </div>
      ...
      <div class="lg:col-span-5 flex flex-col justify-center pl-4 border-l border-gray-100"> 
        <div class="w-full">
          <table class="w-full text-sm mb-2">
            <thead>
              <tr class="text-xs text-[#051960] bg-gray-100/80">
          ...
          <div class="pt-4 text-right text-sm font-bold text-[#051960] border-t border-gray-100 mt-2">
