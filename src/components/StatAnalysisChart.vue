<template>
  <div class="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 font-sans">
    ...
    <div class="flex items-start justify-center mb-10 pb-8 border-b border-gray-200">
      <div v-for="(stat, index) in stats" :key="index" class="flex-1 px-4 flex">
        <div class="text-center w-full">
          ...
          <div class="text-sm font-medium text-[#64748b] mt-1">{{ stat.label }}</div>
        </div>
    ...
    <div class="relative h-64 mb-12"> 
      ...
      <div class="absolute -left-2 top-1/2 -translate-y-1/2 -rotate-90 text-sm text-[#64748b] font-medium whitespace-nowrap z-20 origin-center">
        {{ yAxisLabel }}
      </div>

      <div class="absolute left-12 top-0 bottom-0 flex flex-col justify-between text-[13px] text-[#64748b] font-medium z-10 pb-[1px]">
      ...
      <div class="ml-24 h-full border-b border-gray-300 relative">
        ...
        <div class="flex items-end justify-around h-full relative z-10 px-2">
          <div v-for="(item, index) in chartData" :key="index" class="flex flex-col items-center w-full relative group h-full justify-end">
            ...
            <div 
              class="mb-2 bg-[#051960] text-white text-[14px] font-semibold px-2 py-0.5 rounded-md shadow-sm relative -top-1 z-20 transition-all duration-200 ease-out transform"
            ...
            <span class="absolute top-full mt-3 text-[13px] text-[#64748b] font-medium text-center w-full whitespace-nowrap">
              {{ item.label }}
            </span>
      ...
      <div class="absolute -bottom-14 left-0 right-0 text-center text-sm text-[#64748b] font-medium ml-24 mt-2">
        {{ xAxisLabel }}
      </div>
    </div>
  </div>
